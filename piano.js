...
function drawNote(noteNum) {
     ...
     if (isSharp(noteNum)) {
          document
               .querySelector(".accidental")
               .classList.add("sharp" + noteNum);
     }
     if (isSpecialLineNote(noteNum)) {
          document
               .querySelector("#whole-note")
               .setAttribute("src", "./images/wholenoteline.svg");
          document
               .querySelector("#whole-note")
               .classList.add("whole-note-line");
     } else {
          document
               .querySelector("#whole-note")
               .setAttribute("src", "./images/wholenote.svg");
     }
}
     ...
     if (note >= 41 && note <= 47) {
          ...
          window.setTimeout(function () {
               if (noteIndex < level1Notes.length - 1) {
                    ...
                    document
                         .querySelector(".whole-note")
                         .classList.remove("note" + level1Notes[noteIndex - 1]);
                    drawNote(level1Notes[noteIndex]);
               } else {
                    document.querySelector(".note-info").textContent =
                         "You played " +
